...
- [ ] api `OnboardingModule`:
    ...
    - `GET /api/onboarding/detect-tools` — runs detection for git, claude, codex, gemini, gh; returns
      `{ git: { name, email }?, claude: { version, subscription }?, codex: …, gemini: …, gh: { authenticated, user? } }`
- [ ] Web flow:
    - On boot, if
      `onboarding.completedAt` not set → render onboarding overlay (Vaul drawer from the right, takes full center column)
    ...
    - **Step 2 — Connect Tools**:
        - Git: shows current `user.name`/`user.email`; if missing, walks user through
          `git config --global` commands with copy buttons; "Re-check"
    ...
    - **Step 4 — Add First Project**:
        ...
        - On success, show a "Project Settings preview" with defaults (plan storage = auto, branch template =
          `pixler/<workspace>`)
    - **Step 5 — Telemetry consent**:
        - Single toggle "Help improve Pixler by sharing anonymous usage data" — **checked by default
          ** per SPEC §11.1 step 5
...
- [ ] **Post-onboarding nudge
  **: empty workspaces view shows a single CTA "+ Create your first workspace" — opens a guided variant of the New Workspace dialog with inline hint balloons that fade after first use
