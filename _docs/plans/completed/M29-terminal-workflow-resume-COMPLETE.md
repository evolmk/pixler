...
**Modified:** 2026-05-25
**Current Status:** All 4 sprints complete — `pnpm -w typecheck` clean across all packages.
...
- `apps/web/src/hooks/useWorkflowState.ts` (extended: awaiting_run status, step-prompt/step-advanced handlers, mutation hooks: useMarkStepDone, usePauseStep, useSendToTerminal, useRetryStep)
- `apps/web/src/components/WorkflowStepIndicator.tsx` (awaiting_run styling)
- `apps/web/src/components/StepPromptAccordion.tsx` (new — 3-line preview, expand, copy, Send to terminal)
- `apps/web/src/components/RetryStepDialog.tsx` (new — context textarea + checkpoint restore checkbox)
- `apps/web/src/components/WorkflowRunPanel.tsx` (new — step list, prompt accordion, Mark done, Stop, Retry)
- `apps/web/src/components/ChatPane.tsx` (WorkflowRunPanel injected when awaiting_run step exists)

**Issues Encountered:**

- None

**Verify:** `pnpm -w typecheck` — 10/10 tasks pass.
