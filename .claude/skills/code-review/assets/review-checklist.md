...
- Non-null assertion (`!`) used to silence a real undefined path
- Empty catch blocks
- Magic numbers/strings
- Copy-pasted code blocks (extract to `packages/` or a shared util)
- Missing null checks
- Hardcoded URLs/credentials/Auth0 IDs
- Hardcoded Tailwind values where a design token or design system primitive exists
- NestJS route missing `@Public()` decision (default must require JWT)
