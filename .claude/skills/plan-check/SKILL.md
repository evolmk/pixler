...
1. **List active plans.** Glob `_docs/plans/*.md` (exclude `.TEMPLATE-*`). Skip
   `_docs/plans/completed/` unless the user asks to see completed plans.
...
4. **If asked about a specific plan by name:** read the full file and show sprint-by-sprint
   ...
   pending sprint/task.
5. **If asked to show completed plans:** glob `_docs/plans/completed/*.md` and list name +
   completion status. Pixler convention: completed plans end with `-COMPLETE.md` and have an entry
   in `_docs/plans/completed/INDEX.md`.
