...
export class DatabaseService implements OnModuleInit {
  ...
  private runMigrations() {
    ...
    if (currentVersion < 6) {
      const sql = readFileSync(join(migrationsDir, '0006_plans.sql'), 'utf-8');
      this.db.exec(sql);
    }
    if (currentVersion < 7) {
    ...
    if (currentVersion < 10) {
      const sql = readFileSync(join(migrationsDir, '0010_crashes.sql'), 'utf-8');
      this.db.exec(sql);
    }
    if (currentVersion < 11) {
      const sql = readFileSync(join(migrationsDir, '0011_model_registry.sql'), 'utf-8');
      this.db.exec(sql);
    }
    if (currentVersion < 12) {
      const sql = readFileSync(join(migrationsDir, '0012_workflow_runs.sql'), 'utf-8');
      this.db.exec(sql);
    }
  }
