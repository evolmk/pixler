...
- **Sticky save bar**: Fix Save/Cancel to bottom on mobile (`fixed bottom-0 inset-x-0 lg:hidden`), add `pb-16 lg:pb-0` to page content.
- **Touch targets**: Min `h-11` for checkboxes/switches, `py-3` min for subcollection rows, `size="icon-sm"` min for action buttons.
