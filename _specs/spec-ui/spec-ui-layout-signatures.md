...
**When to read:
** Building any app page, especially hero sections, detail pages, and navigation pages. These are THE brand-defining patterns -- use them consistently.

---

## 1. Overline + Brand-Rule + Display Number

Used in project overviews, workspace heroes, and "stat spotlight" sections. The three-part vertical stack is a *
*signature** — it identifies Pixler pages at a glance.
...
<div className="flex flex-wrap gap-2">
    {stats.map((stat) => (
        <div key={stat.label}
             className="flex flex-col items-center min-w-[80px] px-3 pt-2 pb-1 border border-border rounded-md bg-card">
...
- Label: tiny uppercase (`text-[10px] uppercase tracking-wide text-muted-foreground`) — NOT the
  `eyebrow` utility (too prominent for inline chip labels).
...
- Button hover:
  `hover:bg-white/10` works on both white and green header backgrounds (transparent, so it reads correctly in both states).
...
A breadcrumb appears at the top of every detail page (project, workspace, terminal session). It ends in the **bold
current item**.
...
> **Cross-links:** `spec-ui-design-system.md` → hover/press states, header signature overview ·
`spec-ui-layout-patterns.md` → where signatures plug into page-level layouts
